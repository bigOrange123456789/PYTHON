...
if __name__ == '__main__':
    ...
    #2.定义模型
    lstm_model = LstmRNN(1, 2, output_size=1, num_layers=1) # 16 hidden units
    loss_function = nn.MSELoss()
    optimizer = torch.optim.Adam(lstm_model.parameters(), lr=1e-2)
    print('model.parameters:', lstm_model.parameters)
    for parm0 in lstm_model.parameters():
        print(parm0.shape,parm0.numel())
    #3.训练
    ...
    plt.figure()
    plt.plot(range(len(x)), x , 'm', label='pre_cos_tst')  
    plt.plot(range(len(x)), y2.view(-1, 1).data.numpy(), 'r--', label='pre_cos_tst')  
    plt.show()