...
class Data():
    def __init__(self,file_path):
        self.df = pd.read_csv(file_path).loc[0:10]#读取数据
        self.X = self.df.values.reshape((-1,28,28)).astype(np.uint8)[:,:,:,None]
...
#data=pd.read_csv(file_path).loc[0:10].values
test_dataset = Data('../input/test.csv')
test_loader = torch.utils.data.DataLoader(dataset=test_dataset, shuffle=False)#shuffle洗牌 打乱数据
...
def showPic(data):
    ...
    cv2.imshow('img0',whc)
    cv2.waitKey(0)
showPic(test_dataset.df.values)
